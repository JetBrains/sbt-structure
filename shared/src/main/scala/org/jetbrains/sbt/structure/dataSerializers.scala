...
trait DataSerializers {
  ...
  implicit val projectDependenciesSerializer: XmlSerializer[Dependencies] = new XmlSerializer[Dependencies] {
    override def serialize(what: Dependencies): Elem =
      <projects>
        <forTestSources>
          {what.test.map(_.serialize)}
        </forTestSources>
        <forProductionSources>
          {what.production.map(_.serialize)}
        </forProductionSources>
      </projects>

    override def deserialize(what: Node): Either[Throwable, Dependencies] = {
  ...
  implicit val dependencyDataSerializer: XmlSerializer[DependencyData] = new XmlSerializer[DependencyData] {
    ...
    override def deserialize(what: Node): Either[Throwable,DependencyData] = {
      ...
      (what \ "projects").deserializeOne[Dependencies].right.map { projectDependencies =>
