...
class SbtSourcesIntegrationTest extends AnyFreeSpec {
  ...
  "dumpSbtSourcesTo" - {
    "should run as a dynamically loaded plugin and report source archives for every loaded build" in {
      ...
      // A global plugins directory under the shared sbt global base. We must remove it after the
      // test: otherwise sbt would load it as a global plugin in every
      // subsequent build that shares this global base (e.g. ExtractStructureIntegrationTest),
      // injecting its compiled classes dir into the extracted `<build>` classpath.
      val globalPluginsDir = runOptions.sbtGlobalBase / "plugins"
